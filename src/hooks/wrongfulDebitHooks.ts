...
export const wrongfulDebitHooks: ResourceHooks = {
	beforeCreate: [
		async (ctx: HookContext) => {
			...
			if (!shift) throw new Error(t('app:shifts.errors.notFound'));
			...
			if (!seesTeamWideData(ctx.user?.role) && String(shift.agent?.id ?? shift.agent) !== String(ctx.user?.id)) {
				throw new Error(t('app:wrongfulDebits.errors.notOwnShift'));
			}

			const card = await em.findOne(Card, { id: data.card });
			if (!card) throw new Error(t('app:wrongfulDebits.errors.cardNotFound'));
			if (String(card.group?.id ?? card.group) !== String(shift.cycle.group?.id ?? shift.cycle.group)) {
				throw new Error(t('app:wrongfulDebits.errors.cardWrongGroup'));
			}
	...
	beforeUpdate: [
		async (ctx: HookContext) => {
			if (!isAdminLike(ctx.user?.role)) {
				throw new Error(t('app:wrongfulDebits.errors.updateAdminLikeOnly'));
			}
